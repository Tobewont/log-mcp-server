...
> 下载的 `limit` 是**每租户**上限，受 `LOG_MAX_LIMIT` 限制；多租户下载的总条数可能超过单个 `limit`。多 Loki 集群的服务端 `max_entries_limit_per_query` 可能不同；未指定 `instance` 时，低限额集群会按自身上限重试，不应把其它高限额集群的下载上限降到低限额。用户已知目标集群时，优先传 `instance`，不要扇出到无关集群。要拉更多，缩小时间窗多次调用。看到响应里有"may have been truncated"就是某个租户触顶了，提示用户继续缩小时间窗。
